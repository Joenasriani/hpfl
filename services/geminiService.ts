        ...
        **Step 4: Detail Product & Technical Specifications**
        ...
        - **MANDATORY SVG Design & Execution Protocol:**
            ...
            **Design Rules (The "Checklist")**
            You must adhere to every single rule listed below without exception:
            1. **Canvas Fit & ViewBox:** The entire diagram must fit perfectly within a responsive SVG canvas. You MUST define a \`viewBox\` attribute on the root \`<svg>\` element. All visual elements must be contained within this viewBox with a comfortable margin of at least 10px from all edges.
            2. **Strict No-Overlap Policy:** All elements (arrows, connectors, shapes, text, icons) must be clearly separated. No element may overlap, touch, or cross over any text or any other line. This is a critical rule.
            3. **Text Containment & Padding:** All text inside a shape (e.g., a box or circle) must be fully contained within that shape's boundaries. There must be a clear internal margin (padding) of at least 5px on all sides so that no part of any letter touches the shape's outline.
            4. **Perfect Alignment & Distribution:** All text within shapes must be perfectly centered, both horizontally and vertically. The entire diagram must be neat, well-organized, and logically structured. Elements must be aligned on a grid and distributed with consistent, balanced spacing for an uncluttered, professional appearance. Use symmetry where appropriate.
            5. **Clear Arrow/Connector Paths:** All connectors (arrows) must point clearly from one shape's outline to another shape's outline. They should not float vaguely or overlap the text within a shape. Paths should be simple (straight lines or clean curves) and avoid crossing other connector lines where possible.
            6. **Correct Multi-line Text Rendering:** To prevent text from jumbling and overlapping, you MUST handle multi-line text correctly. For any text that needs to wrap inside a shape, break it into separate lines. Each line MUST be its own \`<tspan>\` element within the parent \`<text>\` tag. Use \`dy="1.2em"\` on subsequent \`<tspan>\` elements to create vertical line breaks. This is mandatory. Example: \`<text x="100" y="50" text-anchor="middle" dominant-baseline="central"><tspan>First Line</tspan><tspan x="100" dy="1.2em">Second Line</tspan></text>\`. Using \`dominant-baseline="central"\` can help with vertical centering.
            
            **Execution Process**
            Your operational flow for generating each diagram must be as follows:
            1. **Generate Draft:** First, you will generate a draft of the requested diagram, considering all design rules.
            2. **STOP (Internal Verification):** Before finalizing the SVG, you must stop and enter a verification phase.
            3. **Perform Rigorous Quality Check:** You will internally review your draft against every single rule in the "Design Rules" checklist above. Be extremely critical. Check for overlaps, misalignments, text touching borders, and canvas fit.
            4. **Self-Correct:**
                - If the draft fails even one rule, you must discard that draft entirely.
                - You will then regenerate the diagram from scratch, focusing specifically on correcting the identified failure. Do not simply tweak the failed draft.
            5. **Repeat Loop:** You will repeat this "Generate -> Check -> Self-Correct" loop as many times as necessary until you have a final version that passes 100% of the Design Rules flawlessly.
